...
//      $('.topicItem').perfectScrollbar();
      ...
      $("#category_manage").on("click","#level2 ul",function() {
          $(".add").hide();
      })
...
function multi_add_new()
{
	...
	//跳转添加第一级jsp处理
	var url="../category/multi_category_add_new2018.jsp?itemid="+itemid+"&channelid="+ChannelID+"&level="+level+"&index="+index;
		dialog.setWidth(935);
...
function multi_add()
{
	...
	{
		//跳转到添加子分类jsp进行处理
		var url="../category/multi_category_add2018.jsp?itemid="+itemid+"&channelid="+ChannelID+"&level="+level+"&index="+index;
		dialog.setWidth(935);
		dialog.setHeight(620);
		dialog.setUrl(url);
...
function multi_edit()
{
	...
	{
		//跳转到处理编辑的jsp
		var url="../category/multi_category_edit2018.jsp?itemid="+itemid+"&channelid="+ChannelID;
		//dialog.setWidth(635);
...
function doupclick(itemid,hasNextLevel) {
  if(hasNextLevel==0){
      ...
      $(".c_m_box ul li[itemid="+itemid+"]").attr("hasNext",1);
  }
