		...
		function selectItem()
		{
			if(globalid===0)
			{
				...
				$.getJSON("../content/document_add_temp.jsp?LinkChannelID="+LinkChannelID+"&tempID="+tempIDInDocument,function(o){
					...
					var url="../custom/select_child_channel_tree2018_pxjdk.jsp?ChannelID="+ChannelID+"&LinkChannelID="+LinkChannelID+"&GlobalID="+tempIDInTable+"&fieldgroup="+fieldgroup;
					window.open(url);
				});
			}
			else
			{
				//var url="../content/select_child_item2018.jsp?ChannelID="+ChannelID+"&LinkChannelID="+LinkChannelID+"&GlobalID="+globalid+"&fieldgroup="+fieldgroup;
				var url="../custom/select_child_channel_tree2018_pxjdk.jsp?ChannelID="+ChannelID+"&LinkChannelID="+LinkChannelID+"&GlobalID="+globalid+"&fieldgroup="+fieldgroup;
				window.open(url);
			}
		}
		...
		<div class="card bd-0 shadow-base">
			<table class="table mg-b-0 <%=listType==2?"viewpane_pic_list":"viewpane_tbdoy"%>" id="content-table">
				...
				<tr id="oTable_th">
					...
					<th class="tx-12-force tx-mont tx-medium">端口号</th>
					<th class="tx-12-force tx-mont tx-medium hidden-xs-down">上联</th>
					<th class="tx-12-force tx-mont tx-medium hidden-xs-down">下联</th>
...
//System.out.println("ids="+ids);
					...
					for (int i=0;i<arrayList_ChildGlobalID.size();i++){
						...
						String UpPort	= doc.getValue("UpPort");
						String DownPort	= doc.getValue("DownPort");
				...
				<tr No="<%=j%>" ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"   status="<%=Status%>" GlobalID="<%=GlobalID%>"  Channelid=<%=sourcechannelid%> id="item_<%=id_%>" class="tide_item">
					...
					<td class="hidden-xs-down"><%=UpPort%>	</td>
					<td class="hidden-xs-down"><%=DownPort%>	</td>
