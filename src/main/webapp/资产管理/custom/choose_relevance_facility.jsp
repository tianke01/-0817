...
<%
    ...
    System.out.println("----文件夹新增----"+fieldname);
    id = CmsCache.getChannel(fieldname).getId();
            ...
            <div class="card bd-0 shadow-base">
                ...
                <table class="table mg-b-0  ui-sortable" id="content-table">
                    ...
                    <tr>
                        ...
                        <th class="tx-12-force tx-mont tx-medium header">标题</th>
                        <th class="tx-12-force tx-mont tx-medium header">网卡IP</th>
                        <th class="tx-12-force tx-mont tx-medium header">网卡链接</th>
                        <th class="tx-12-force tx-mont tx-medium header">tcp连接数</th>
                    ...
                    <%
                        ...
                        while(Rs.next())
                        {
                            ...
                            String Link =convertNull(Rs.getString("Link"));
                            int TcpNum = Rs.getInt("TcpNum");
                    ...
                    <tr class="tide_item" No="<%=j%>"   ItemID="<%=id_%>" OrderNumber="<%=OrderNumber%>"  GlobalID="<%=GlobalID%>" id="jTip<%=j%>_id">
                        <td class="valign-middle" align="center" valign="middle">
                            ...
                            <label class="rdiobox mg-b-0 mg-l-10">
                                <input type="radio" name="choose" value="<%=id_%>" title="<%=Title%>" IP="<%=IP%>" Link="<%=Link%>" TcpNum="<%=TcpNum%>"><span></span>
                            </label>
                            <%}else if(type==2){%>
                            <label class="ckbox mg-b-0 mg-l-10">
                                <input type="checkbox" name="choose" value="<%=id_%>" title="<%=Title%>" IP="<%=IP%>" Link="<%=Link%>" TcpNum="<%=TcpNum%>"><span></span>
                            </label>
                            <%}%>
                        </td>
                        <td class="hidden-xs-down" valign="middle"><%=Title%></td>
                        <td class="hidden-xs-down" valign="middle"><%=IP%></td>
                        <td class="hidden-xs-down" valign="middle"><%=Link%></td>
                        <td class="hidden-xs-down" valign="middle"><%=TcpNum%></td>
        ...
        function next()
        {
            if(type==1){
                ...
                if (checkedCheckbox.length == 0) {
                    ...
                    var IP = checkedCheckbox.attr("IP");
                    var Link = checkedCheckbox.attr("Link");
                    var TcpNum = checkedCheckbox.attr("TcpNum");
                    //console.log(personnelArr);
                    //top.TideDialogClose({suffix: '_1', recall: true, returnValue: {type:1,iconArr:iconArr}});
                    parent.$("#<%=fieldname%>").val(IP+" - "+title);
                ...
                if (checkedCheckbox.length == 0) {
                    ...
                    parent.$("#<%=fieldname%>").val(IP+" - "+title);
                    //parent.$("#tr_<%=fieldname%>").css("display","none");
                    //parent.$("#<%=fieldname%>Name").val(title);
                    //top.TideDialogClose({suffix: '_1', recall: true, returnValue: {type:1,iconArr:iconArr}});
