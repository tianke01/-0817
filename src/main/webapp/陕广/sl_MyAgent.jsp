...
<%
    ...
    String S_taskType=getParameter(request,"taskType");//任务名称
    int status=getIntParameter(request,"status");//审核状态  （0：未审核  1：已审核）
    ...
    Map map1 = getMyApprove(map,"", userInfo.getId(), status, request, S_taskType, page1, limit, S_Title, S_startDate, S_endTime);
    if(!map.isEmpty()){
...
<%!
    ...
    public Map getMyApprove(Map map,String approveItemIds, int UserId, int status, HttpServletRequest request, String application, int pagenum, int pagesize, String S_Title, String S_startDate, String S_endDate) throws MessageException, SQLException, JSONException {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            map1.put("S_taskType", S_taskType);
            al.add(map1);
        }
        tu.closeRs(rs);
        tu.closeRs(rs1);
        ...
        map.put("list", al);
        map.put("sum", totalNumber);
        map.put("pageNum", totalPageNumber);
        return map;
    }
